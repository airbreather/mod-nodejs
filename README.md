...
2. (Optional) Looking for `libnode.pdb` files to put next to your `libnode.dll` files? There's a `libnode-windows-pdb-*.7z` archive right next to the main one in [the main release](https://github.com/airbreather/mod-nodejs/releases). CMake doesn't download the PDBs automatically because they're huge and not everybody would benefit from them.
